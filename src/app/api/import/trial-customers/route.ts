...
export async function POST() {
  try {
    ...
    const result = await db.insert(customerDB).values(trialCustomers).returning({ 
      id: customerDB.id,
    ...
    return NextResponse.json({
      success: true,
      message: `Successfully imported ${result.length} trial customers`,
      data: result,
    });
    ...
    return NextResponse.json({
      success: false,
      message: 'Failed to import trial customers',
...
export async function GET() {
  return NextResponse.json({
    success: true,
    message: 'Trial customer import endpoint ready. Use POST to import 20 trial customers.',
    trialCustomersCount: trialCustomers.length,
  });
}